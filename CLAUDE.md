# Claude Code Project Instructions — age-kotlin

## Project Overview

This is **age-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`age`](https://crates.io/crates/age).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/age/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/age>
